/** Errors */
import "views/client/errors/_errors.php";
import "views/client/errors/404.php";
/** Restricted access */
import "views/client/restricted/index.php";
/** Authentication System */
import "views/client/components/authSystem/loginModal.php";
import "views/client/components/authSystem/forgorPwModal.php";
import "views/client/components/authSystem/registerModal.php";
/** Navigation Component */
import "views/client/components/navigation/_nav.php";
import "views/client/components/navigation/_connexion.php";
import "views/client/components/navigation/_navbar_brand.php";
import "views/client/components/navigation/_settings.php";
/** Phone products brand */
import "views/client/brand/phones/partials/_banner_adds.php";
import "views/client/brand/phones/partials/_banner_area.php";
import "views/client/brand/phones/partials/_blog.php";
import "views/client/brand/phones/partials/_new_products.php";
import "views/client/brand/phones/partials/_product_details.php";
import "views/client/brand/phones/partials/_special_price.php";
import "views/client/brand/phones/partials/_top_sales.php";
/** Clothes Layouts */
import "views/client/layouts/inc/clothes/footer.php";
import "views/client/layouts/inc/clothes/header.php";
import "views/client/layouts/clothes.php";
/** Default Layouts */
import "views/client/layouts/default.php";
import "views/client/layouts/inc/default/footer.php";
import "views/client/layouts/inc/default/header.php";
/** Phones Product HomePage */
import "views/client/brand/phones/index.php";
import "views/client/brand/phones/details.php";
/** Clothes Home page */
import "views/client/brand/clothes/clothing.php";
import "views/client/brand/clothes/shop.php";
import "views/client/brand/clothes/details.php";
import "views/client/brand/clothes/partials/_brand.php";
import "views/client/brand/clothes/partials/_middle_season.php";
/** Shopping cart & whishlist component */
import "views/client/components/shoppingCart/shoppingCart.php";
import "views/client/components/shoppingCart/partials/_shopping_cart.php";
import "views/client/components/shoppingCart/partials/_empty_shopping_cart.php";
import "views/client/components/shoppingCart/partials/_shopping_cart_subTotal.php";
import "views/client/components/shoppingCart/partials/_shopping_whishlist.php";
/** Checkout */
import "views/client/components/checkout/checkout.php";
/** Checkout Progress bar */
import "views/client/components/checkout/partials/progress_bar/_progress_bar.php";
/** Checkout user infos */
import "views/client/components/checkout/partials/cart_summary/_card_summary.php";
import "views/client/components/checkout/partials/cart_summary/_cart_summary_data.php";
import "views/client/components/checkout/partials/cart_summary/_cartSummaryTotal.php";
import "views/client/components/checkout/partials/user_informations/_chk_user_info.php";
import "views/client/components/checkout/partials/user_informations/_user_data.php";
import "views/client/components/checkout/partials/buttons_group/_button_group.php";
import "views/client/components/checkout/partials/user_informations/_contact_title.php";
import "views/client/components/checkout/partials/popup.php";
/** Checkout shipping infos */
import "views/client/components/checkout/partials/shipping_informations/_shipping_infos.php";
import "views/client/components/checkout/partials/shipping_informations/_shipping_data.php";
import "views/client/components/checkout/partials/shipping_informations/_shipping_method.php";
/** Checlout Billing Infos */
import "views/client/components/checkout/partials/billing_informations/_billing_data.php";
import "views/client/components/checkout/partials/billing_informations/_billing_infos.php";
/** Paiements Infos */
import "views/client/components/checkout/partials/paiment_informations/_paiement_data.php";
import "views/client/components/checkout/partials/paiment_informations/_paiment_infos.php";
/**Modals Components */
import "views/client/components/modals/_checkout_add_address.php";
/** Credit Card */
import "views/client/components/credit_card/_cc_front.php";
import "views/client/components/credit_card/_cc_back.php";
/** Todo List */
import "views/client/components/todoList/todo.php";
/** Learning */
import "views/client/learn/learn.php";
/**Email Sending */
import "views/client/components/Email/welcomeTemplate.php";
/** user Account elements */
import "views/client/components/user_account/account.php";
import "views/client/components/user_account/partials/_transaction_menu.php";
// -----------------------------------------------------------------------
// Admin Views
// -----------------------------------------------------------------------

/** Admin Layout */
import "views/admin/layouts/admin.php";
import "views/admin/layouts/inc/admin/footer.php";
import "views/admin/layouts/inc/admin/header.php";
import "views/admin/layouts/inc/admin/topbar.php";
import "views/admin/layouts/inc/admin/sidebar.php";

/** Admin Pages */
import "views/admin/pages/index.php";
// products
import "views/admin/pages/products/allProducts.php";
//Brands
import "views/admin/pages/brands/allCategories.php";
import "views/admin/pages/brands/allBrands.php";
//Users
import "views/admin/pages/users/allUsers.php";
import "views/admin/pages/users/permissions.php";
import "views/admin/pages/users/profile.php";
