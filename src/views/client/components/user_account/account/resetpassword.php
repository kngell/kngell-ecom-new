...
$this->start('head'); ?>
<!-------Costum-------->
<link href="<?= $this->asset('css/custom/client/users/account/resetpassword', 'css') ?? ''?>" rel="stylesheet"
    type="text/css">
<?php $this->end(); ?>
<?php $this->start('body'); ?>
<main id="main-site">
                ...
                <div class="card-group" id="form-wrapper">
                    <div class="card rounded-right p-4" style="flex-grow:2;">
                        ...
                        <form action="#" method="post" class="px-3 needs-validation" novalidate id="resetpass-frm">
                            <?=FH::csrfInput('csrftoken', $this->token->generate_token(8, 'resetpass-frm')); ?>
                            <input type="hidden" name="user_data" value="<?=$_GET['url']?>">
...
<!----------custom--------->
<script type="text/javascript" src="<?= $this->asset('js/custom/client/users/account/resetpassword', 'js') ?? ''?>">
</script>
<?php $this->end();
