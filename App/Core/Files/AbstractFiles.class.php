...
abstract class AbstractFiles
{
     ...
     * @param int $flags (optional) ORed flags using PHP's FILE_* constants (see manual of file_get_contents).
    ...
    public static function getFileContents(string $pathAndFilename, int $flags = 0, $context = null, ?int $offset = null, int $maximumLength = -1)
    {
        ...
        try {
            if ($maximumLength > -1) {
                $content = file_get_contents($pathAndFilename, (bool) $flags, $context, $offset, $maximumLength);
            } else {
    ...
    public static function bytesToSizeString($bytes, ?int $decimals = null, ?string $decimalSeparator = null, ?string $thousandsSeparator = null): string
    {
        ...
        return sprintf('%s %s', number_format(round($bytes, 4 * $decimals), $decimals, $decimalSeparator, $thousandsSeparator), self::$sizeUnits[$pow]);
    }
